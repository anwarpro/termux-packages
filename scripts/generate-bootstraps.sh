...
create_bootstrap_archive() {
  ...
  (
    ...
    #rename all binary file to libbin_name.so
    (
      cd "bin"
      for filename in *; do mv "$filename" "libbin_${filename}.so"; done
    )
      if ! ${ONLY_ADDITIONAL}; then
            mv "lib/apt/apt-helper" "bin/liblib_apt_apt-helper.so"
            mv "libexec/termux/command-not-found" "bin/liblibexec_termux_command-not-found.so"

            (
              cd "lib/apt/methods"
              for filename in *; do mv "$filename" "../../../bin/liblib_apt_methods_${filename}.so"; done
            )
      fi

    (
      cd "bin"
      zip -r9 "${BOOTSTRAP_TMPDIR}/bins-${1}.zip" ./*
      rm -rf ./*
    )
  ...
  mv -f "${BOOTSTRAP_TMPDIR}/bins-${1}.zip" ./
...
show_usage() {
  ...
  echo " --onlyadditional                 Generate bootstrap archives for additional packages"
  echo
